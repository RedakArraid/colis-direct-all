...
interface MyAddressesPageProps extends BasePageProps {
  // onNavigate non utilisé, supprimé pour cohérence
}

function MyAddressesPage({}: MyAddressesPageProps) {
  const { user } = useAuth();
  ...
  const loadAddresses = async () => {
    ...
    setLoading(true);
    try {
      const { data, error } = await api.getSenderAddresses();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        const { error } = await api.updateSenderAddress(editingId, formData);
        if (error) throw new Error(error);
      } else {
        const { error } = await api.createSenderAddress(formData);
        if (error) throw new Error(error);
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { error } = await api.deleteSenderAddress(id);
      if (error) throw new Error(error);
      await loadAddresses();
    } catch (err: any) {
  ...
  const handleSetDefault = async (id: string) => {
    try {
      const { error } = await api.updateSenderAddress(id, { is_default: true });
      if (error) throw new Error(error);
      await loadAddresses();
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-[#F6F7F9] py-4 sm:py-6 md:py-8 px-3 sm:px-4 md:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-[#1A1A1A] mb-1 sm:mb-2">Mes adresses d'expédition</h1>
            <p className="text-sm sm:text-base text-[#6B7280]">Gérez vos adresses pour faciliter vos envois</p>
          </div>
          <button
          ...
          <div className="bg-white rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.06)] p-4 sm:p-6 mb-4 sm:mb-6">
            <div className="flex items-center justify-between mb-3 sm:mb-4">
              <h2 className="text-lg sm:text-xl font-extrabold text-[#1A1A1A]">
                {editingId ? 'Modifier l\'adresse' : 'Nouvelle adresse d\'expédition'}
              </h2>
              <button
                onClick={resetForm}
        ...
        <div className="bg-white rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.06)]">
          {loading ? (
            <div className="p-8">
              <LoadingSpinner message="Chargement de vos adresses..." />
            </div>
          ) : addresses.length === 0 ? (
            <div className="p-8 text-center text-[#6B7280]">
              <MapPin className="w-12 h-12 mx-auto mb-4 text-[#9CA3AF]" />
              <p className="mb-2">Aucune adresse d'expédition enregistrée</p>
              <p className="text-sm text-[#9CA3AF]">Ajoutez votre première adresse pour faciliter vos envois</p>
            </div>
          ) : (
