...
function renderProfile() {
  ...
  el.innerHTML = `
    ...
    <div class="stats-row" style="margin:16px">
      ...
      <div class="stat-card" style="background:#E6F6EC;border-color:#86EFAC" onclick="setShipmentsTab('delivered');Router.navigate('shipments')" style="cursor:pointer">
...
function renderSigninFields() {
  return `
    ...
    <div id="login-error" style="display:none;background:#FEF2F2;border-left:3px solid #DC2626;border-radius:10px;padding:12px;margin-bottom:14px;font-size:13px;color:#DC2626;display:flex;align-items:center;gap:8px"></div>
...
function renderCreateShipment() {
  ...
  el.innerHTML = `
    ...
    <div class="step-indicator">
      ${[1,2,3,4].map(s => `<div class="step-dot ${s < createStep ? 'done' : s === createStep ? 'active' : ''}"></div>`).join('')}
    </div>
    <div style="text-align:center;font-size:11px;color:#9CA3AF;margin-top:-10px;margin-bottom:4px">Étape ${createStep} sur 4</div>
  ...
  return `
    ...
    <div class="form-group"><label class="form-label">Commune de départ *</label>
      <select class="form-select" id="s-commune">
        ...
        ${['Abidjan Plateau','Abidjan Cocody','Abidjan Yopougon','Abidjan Abobo','Abidjan Marcory','Abidjan Koumassi','Abidjan Treichville','Bouaké','Yamoussoukro','Korhogo','San-Pédro','Daloa','Man','Abengourou','Divo','Gagnoa'].map(c => `<option value="${c}" ${createData.sender_commune === c ? 'selected' : ''}>${c}</option>`).join('')}
      </select>
    </div>
    <div class="form-group"><label class="form-label">Adresse complète</label>
      <input class="form-input" id="s-address" placeholder="Rue, quartier, bâtiment…" value="${createData.sender_address || ''}" /></div>
  ...
  return `
    ...
    <div class="form-group"><label class="form-label">Commune d'arrivée *</label>
      <select class="form-select" id="r-commune">
        <option value="">Choisir une commune</option>
        ${['Abidjan Plateau','Abidjan Cocody','Abidjan Yopougon','Abidjan Abobo','Abidjan Marcory','Abidjan Koumassi','Bouaké','Yamoussoukro','Korhogo','San-Pédro','Daloa','Man','Abengourou','Divo','Gagnoa','Soubré','Boundiali','Ferkessédougou'].map(c => `<option value="${c}" ${createData.recipient_commune === c ? 'selected' : ''}>${c}</option>`).join('')}
      </select>
    </div>
...
function stepPackage() {
  ...
  return `
    ...
    <div class="size-grid">
      ${[
        { id: 'petit',  label: 'Petit',  icon: '📦', sub: '< 3 kg',   price: '600 F' },
        { id: 'moyen',  label: 'Moyen',  icon: '🗃️', sub: '3–10 kg',  price: '1 500 F' },
        { id: 'grand',  label: 'Grand',  icon: '📫', sub: '> 10 kg',  price: '2 500 F' },
      ].map(s => `
        <div class="size-option ${size === s.id ? 'selected' : ''}" onclick="createData.package_type='${s.id}';document.getElementById('cs-content').innerHTML=stepPackage()">
...
function stepSummary() {
  const sizes = { petit: { label: 'Petit (< 3kg)', price: 600 }, moyen: { label: 'Moyen (3–10kg)', price: 1500 }, grand: { label: 'Grand (> 10kg)', price: 2500 } };
  const sz = sizes[createData.package_type || 'moyen'];
  const pickupFee = createData.pickup_method === 'home_pickup' ? 500 : 0;
  const deliveryFee = createData.home_delivery ? 500 : 0;
  const total = sz.price + pickupFee + deliveryFee;
  ...
  return `
    ...
    <div class="card" style="padding:16px;background:#FFF3E8;border-color:#FFD4A8">
      <div style="display:flex;align-items:center;justify-content:space-between">
        <div style="flex:1">
          ...
          <div style="font-size:12px;color:#6B7280">${createData.sender_commune}</div>
        </div>
        ...
        <div style="flex:1;text-align:right">
          ...
          <div style="font-size:12px;color:#6B7280">${createData.recipient_commune}</div>
        </div>
      </div>
    </div>
    ...
    <div class="card" style="padding:0 14px">
      <div class="info-row"><span class="info-key">Format</span><span class="info-val">${sz.label}</span></div>
    ...
    <!-- Prix -->
    <div class="card" style="padding:0 14px">
      <div class="info-row"><span class="info-key">Tarif de base</span><span class="info-val">${sz.price.toLocaleString()} FCFA</span></div>
...
function nextStep() {
  if (createStep === 1) {
    ...
    if (!createData.sender_first_name || !createData.sender_last_name || !createData.sender_commune) {
      Toast.show('Remplissez les champs obligatoires (*)', 'warning'); return;
    }
    ...
    if (!createData.recipient_first_name || !createData.recipient_last_name || !createData.recipient_commune) {
      Toast.show('Remplissez les champs obligatoires (*)', 'warning'); return;
    }
...
async function submitShipment() {
  ...
  const sizes = { petit: 600, moyen: 1500, grand: 2500 };
  ...
  const newS = {
    ...
    price: sizes[createData.package_type || 'moyen'] + (createData.pickup_method === 'home_pickup' ? 500 : 0) + (createData.home_delivery ? 500 : 0),
    weight: createData.package_type === 'petit' ? 1 : createData.package_type === 'grand' ? 15 : 5,
...
function clearCache() {
  Sheet.open(`
    <div style="text-align:center;padding:8px 0 12px">
      ...
      <div style="display:flex;gap:10px">
        <button class="btn btn-outline btn-full" onclick="Sheet.close()">Annuler</button>
        <button class="btn btn-primary btn-full" onclick="Sheet.close();Toast.show('Cache vidé (42 Mo libérés)', \'success\')">Vider</button>
      </div>
    </div>
  `, 'Cache');
}
...
function deleteAccount() {
  Sheet.open(`
    <div style="text-align:center;padding:8px 0 12px">
      ...
      <div style="display:flex;gap:10px">
        <button class="btn btn-outline btn-full" onclick="Sheet.close()">Annuler</button>
        <button class="btn btn-full" style="background:#DC2626;color:#fff;border-radius:14px" onclick="doLogout();Sheet.close();Toast.show('Compte supprimé', \'success\')">Supprimer</button>
      </div>
    </div>
  `, 'Suppression');
}
